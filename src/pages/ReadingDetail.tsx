...
const zhuzhi_level1: Question[] = [
  ...
  },
  {
    ...
    options: [
      ...
      'B) Several different species of insects use shimmering to defend against hornets.',
      'C) Researchers think that shimmering in giant honeybees is a specific defense against hornets.',
      'D) Researchers are unsure how giant honeybees defend against predators larger than hornets.',
    ],
    answer: 2,
  },
  {
    ...
    options: [
      ...
      "B) The results of Barve's study suggest that the ability of birds to withstand cold temperatures is determined more strongly by feather length than feather structure, challenging an established belief.",
      'C) Barve found an association between habitat temperature and feather structure among Himalayan songbirds, lending new support to a general prediction.',
      'D) Barve discovered that songbirds have adapted to their environment by growing feathers without flat and smooth sections, complicating an earlier hypothesis.',
    ],
    answer: 2,
  },
  {
...
export default function ReadingDetail() {
  ...
  const [expandedPassage, setExpandedPassage] = useState<Record<number, boolean>>({})
  ...
  const togglePassage = (qId: number) => {
    setExpandedPassage(prev => ({ ...prev, [qId]: !prev[qId] }))
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-3xl mx-auto">
        {/* Header */}
        <Link to="/knowledge" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-emerald-600 transition-colors mb-6">
        ...
        <p className="text-gray-500 mb-2 text-sm">共 {questions.length} 道题</p>
        ...
        <div className="space-y-8">
          {questions.map((q, idx) => {
            ...
            const isPassageExpanded = expandedPassage[q.id] ?? true

            return (
              <div key={q.id} className="bg-white rounded-2xl border p-6 shadow-sm">
                {/* 题号 */}
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-7 h-7 rounded-full bg-emerald-100 text-emerald-700 text-sm font-bold flex items-center justify-center">
                    {idx + 1}
                  </span>
                  {isRevealed && (
                    userAnswer === q.answer
                      ? <span className="flex items-center gap-1 text-green-600 text-sm font-semibold"><CheckCircle size={16} /> 正确</span>
                      : <span className="flex items-center gap-1 text-red-500 text-sm font-semibold"><XCircle size={16} /> 错误</span>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-gray-400 hover:text-emerald-600 transition-colors mb-2 font-medium"
                  >
                    {isPassageExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                    {isPassageExpanded ? '收起文章' : '展开文章'}
                  </button>
                  {isPassageExpanded && (
                    <div className="bg-gray-50 rounded-xl p-4 text-sm text-gray-700 leading-relaxed border border-gray-100 whitespace-pre-line">
                      {q.passage}
                    </div>
                  )}
                </div>
                ...
                {/* 题目 */}
                <p className="text-gray-900 font-semibold mb-4 leading-relaxed">{q.question}</p>
                ...
                <div className="space-y-3 mb-5">
                  {q.options.map((opt, optIdx) => {
                    ...
                    if (!isRevealed) {
                      ...
                      } else if (optIdx === userAnswer && userAnswer !== q.answer) {
                    ...
                    return (
                      <button key={optIdx} className={btnClass} onClick={() => handleSelect(q.id, optIdx)}>
                        {opt}
                      </button>
                    )
                  })}
                ...
                {!isRevealed ? (
                  <button
                    ...
                    className="px-5 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    确认答案
                  </button>
                ) : null}
        ...
        {answeredCount === questions.length && (
          <div className="mt-10 p-6 bg-emerald-50 border border-emerald-200 rounded-2xl text-center">
            ...
            <p className="text-gray-600">得分：<span className="font-bold text-emerald-700">{score} / {questions.length}</span></p>
            <Link to="/knowledge" className="mt-4 inline-block px-6 py-2 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors text-sm">
              返回知识点
            </Link>
          </div>
        )}
